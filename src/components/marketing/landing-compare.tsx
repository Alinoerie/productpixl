  ...
  return (
    <section data-m-scroll className="border-y border-[var(--border)] bg-[var(--ink)] px-4 py-20 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <div data-m-stagger className="mt-12 grid gap-6 lg:grid-cols-2">
          <div data-m-item className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
          ...
          <div data-m-item className="rounded-2xl border border-[var(--accent)]/30 bg-[var(--accent)]/10 p-6">
        ...
        <div data-m-stagger className="mt-10 grid gap-4 md:grid-cols-3">
          {USP_PILLARS.slice(1, 4).map((pillar, i) => {
            const icons = [CreditCard, Globe, Clock] as const;
            const Icon = icons[i] ?? CreditCard;
            return (
              <div key={pillar.id} data-m-item className="flex gap-4 rounded-xl border border-white/10 p-4">
